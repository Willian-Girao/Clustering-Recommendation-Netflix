...
testMatrix = []
test_matrix = open("target_movie.txt", "r")
...
for userr in testMatrix:	
	...
	training_file = 'movieId_X_userId_ratings_matrix.txt'

	# Wanted n closest neighbors
	n_closest_to_target = 1
	...
	print(top_n_closest[0])

	print("Finding match...\n")
	datapoint_file.close()
	datapoint_file = open(training_file, 'r')
	...
	for line in datapoint_file:
		...
		correct_count = 0
		for x in range(0, len(top_n_closest[0])):
			if row_i[x] == top_n_closest[0][x]:
				correct_count = correct_count + 1

		if correct_count == len(top_n_closest[0]):
			print("Match found: " + str(line_index))
