...
for userr in testMatrix:	
	...
	for x in target_user_vector:
		aux.append(x)
		if str(x) != str(0) and target_retrieved:
			targe_movie_idex = count_cr
	...
	for line in datapoint_file:
		...
		target_last_movie_ratings_as_label.append(matrix_row_i[targe_movie_idex])

	X = np.array(matrix_R)
	Y = np.array(target_last_movie_ratings_as_label)

	knn = KNeighborsClassifier(n_neighbors=n_closest_to_target)
	knn.fit(X, Y)
	...
	print(target_user_vector[count_cr])
	# pause = input("aa")
	if cluster_prediction == target_user_vector[count_cr]:
